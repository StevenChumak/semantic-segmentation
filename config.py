...
__C.DATASET.RAILSEM19_DIR = os.path.join(__C.ASSETS_PATH, "railsem19/800-400/")

__C.DATASET.KNOTS_DIR = os.path.join(__C.ASSETS_PATH, "knotDataset/")
__C.DATASET.TRAINRAILS_DIR = os.path.join(__C.ASSETS_PATH, "finalDataset/")

__C.DATASET.BACKGROUND_DIR = os.path.join(
    __C.ASSETS_PATH, "railway-segmentation/data/bgr/"
)
...
__C.BEST_EGO_TRACKBED = 0
__C.BEST_EGO_RAILS = 0


def update_ego_trackbed(trackbed):
    cfg.immutable(False)
    cfg.BEST_EGO_TRACKBED = trackbed
    cfg.immutable(True)


def update_ego_rails(rails):
    cfg.immutable(False)
    cfg.BEST_EGO_RAILS = rails
    cfg.immutable(True)
